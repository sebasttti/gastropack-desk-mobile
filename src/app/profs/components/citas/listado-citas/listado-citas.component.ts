...
import { CitasOptionsDialogComponent } from '../../dialogs/citas-options-dialog/citas-options-dialog.component';
...
import { ResultadoCitaDialogComponent } from '../../dialogs/resultado-cita-dialog/resultado-cita-dialog.component';
...
export class ListadoCitasComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  private mostrarCitas() {
    ...
    const url = environment.apiUrl + '/profs/citas/mostrarCitas/';
  ...
  public solicitarCita() {
    const solicitarCita = this.dialog.open(SolicitarCitasDialogComponent, {
      ...
      data: {
        tipoProceso: this.userLogged$.type - 1,
        profesional_id: this.userLogged$.id
      }
    });

    solicitarCita.afterClosed().subscribe(formd => {
      if (formd) {
        const urlToSend = environment.apiUrl + '/Profs/Citas/solicitarCita/';
  ...
  private emitirResultadoCita(idCita) {
    const resultadoCita = this.dialog.open(ResultadoCitaDialogComponent, {
      width: this.isHandsetValue ? '90%' : '700px',
      autoFocus: false
    });

    resultadoCita.afterClosed().subscribe(data => {
      if (data) {
        data.append('cita_id', idCita);
        const url = environment.apiUrl + '/Profs/Citas/asignarResultadoCita/';
        this.httpRequest
          .postRequest(url, data)
          .pipe(map(result => result as MessageRequest))
          .subscribe(response => {
            if (response.status === 'success') {
              alert('Resultado de la cita agregado con éxito');
              this.mostrarCitas();
            } else {
              console.log(`${response.status}: ${response.message}`);
            }
